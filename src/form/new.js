...
import 'zarm/lib/input/style/index.css';
import 'zarm/lib/cell/style/index.css';
import 'zarm/lib/button/style/index.css';
import 'zarm/lib/message/style/index.css';
import 'zarm/lib/icon/style/index.css';
     ...
     <div style={{margin: '20px'}}>
       <form onSubmit={handleSubmit(onSubmit)}>
         <Cell title="投保人姓名" help={errors.name ? <Message theme="danger" icon={<Icon type="warning-round" size="sm" />}>{errors.name.message || '必填'}</Message> : ''}>
           <Input name="name" onChange={(value) => handleChange('name', value)} placeholder="投保人姓名"  inputRef={register({ required: true })}/>
         </Cell>
         <Cell title="身份证号码" help={errors.applicantCertiNo ? <Message theme="danger" icon={<Icon type="warning-round" size="sm" />}>{errors.applicantCertiNo.message || '必填'}</Message> : ''}>
           <Input name="applicantCertiNo" onChange={(value) => handleChange('applicantCertiNo', value)} placeholder="身份证号码" inputRef={register({ required: true })}/>
         </Cell>
         <Cell title="手机号"  help={errors.applicantPhone ? <Message theme="danger" icon={<Icon type="warning-round" size="sm" />}>{errors.applicantPhone.message || '必填'}</Message> : ''} description={<Button size="sm" onClick={(value) => trigger('applicantPhone')} >发送验证码</Button>}>
           <Input name="applicantPhone" onChange={(value) => handleChange('applicantPhone', value)} placeholder="身份证号码" inputRef={register({ required: true })} />
         </Cell>
         {/* <Cell>
