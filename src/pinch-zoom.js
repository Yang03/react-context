var PinchZoom = (function () {
  ...
  class PointerTracker {
    ...
    _move(event) {
      ...
      const changedPointers = ('changedTouches' in event) ? // Shortcut for 'is touch event'.
        Array.from(event.changedTouches).map(t => new Pointer(t)) : [new Pointer(event)];
  ...
  class PinchZoom extends HTMLElement {
    ...
    scaleTo(scale, opts = {}) {
      ...
      const {
        relativeTo = 'content', allowChangeEvent = false,
      } = opts;
    ...
    setTransform(opts = {}) {
      const {
        scale = this.scale, allowChangeEvent = false,
      } = opts;
      let {
        x = this.x, y = this.y,
      } = opts;
